...
from models import net_dvd_grl_t_noLSTM as net
from models.backbone_dict import backbone_dict
from models.net_dvd_grl import FineNet, FineNet_npic
from models.net_dvd_grl_t_noLSTM_cin_mdc import FastDVDNet
...
def get_domain_loader(args):
    ...
    dataset = ntu_dataset.NTU_dataset(data_root=args.data_root,
                                      indexfile_path=args.domain_target_file,
                                      transform=transform,
                                      crop_size=args.crop_size,
                                      is_testing=False
                                      )
    ...
    # dataset, dataloader
    domain_dataloader = ntu_dataset.get_dataloader(dataset=dataset,
                                                  batch_size=args.batch_size,
                                                  shuffle=True,
                                                  num_workers=8
                                                  )
...
# ==== checkpoint utils ======
def save_checkpoint(state, path, filename, max_keep):
