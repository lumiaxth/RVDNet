...
from models.M_U_Net import M_U_Net_GRL, M_U_Net
...
class FastDVDNet(nn.Module):
    def __init__(self, in_frames=5, color=True, sigma_map=False):
        """
        class initial
    ...
    def forward(self, input):
        ...
        frames = input
        b, c, N, h, w = frames.size() #torch.Size([32, 3, 5, 224, 224])
        #print(frames.size())
        data_temp_stage1 = []
        # first stage
        ...
        return self.block2(data_temp, frames[:, :, N//2, ...]), temporal_domain_sum


if __name__ == '__main__':
    s = b, c, d, h, w = 2, 3, 5, 224, 224
    t = torch.ones(s).cuda()
    ...
    out_f, temporal_domain_feature = F_net(output)
