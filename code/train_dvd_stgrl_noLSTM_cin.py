...
def validate(args, net_C, net_F):
    ...
    for batch_idx, sample in enumerate(test_loader):
        ...
        # model forward
        with torch.no_grad():
            model_out_C, _ = net_C(inp_videos)   # (b, c, d, h, w)
            c_now = time.time()
            time_C.update(c_now - end)
            # print(type(model_out_C))

            model_out_F, _ = net_F(model_out_C,inp_videos)  # (b, c, 1, h, w)
...
def train(args):
    ...
    scheduler_C = StepLR(optimizer_C, step_size=30, gamma=0.1)
    scheduler_F = StepLR(optimizer_F, step_size=30, gamma=0.1)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        losses_C_main = AverageMeter()
        losses_F_main = AverageMeter()
        losses_target_domain = AverageMeter()
        losses_source_domain = AverageMeter()
        losses_target_domain_F = AverageMeter()
        losses_source_domain_F = AverageMeter()
        ...
        for batch_idx, (sample, domain_sample) in enumerate(zip(train_loader,domain_loader)):
            ...
            if args.freeze_net_C:
                ...
                target_out_C, target_domain_feature = net_C(target_videos)
            ...
            losses_C_main.update(loss_C_main.item(),batch_size)
            losses_source_domain.update(loss_source_domain.item(),batch_size) 
            losses_target_domain.update(loss_target_domain.item(),batch_size) 
            ...
            if args.freeze_net_F:
                ...
                model_out_F, source_domain_feature_F = net_F(model_out_C.detach(),in_videos_C)
                _, target_domain_feature_F = net_F(target_out_C.detach(),target_videos)

                # temporal domain adaptation loss
                source_domain_gt = torch.Tensor([[[0.]]]).cuda()
                target_domain_gt = torch.Tensor([[[1.]]]).cuda()
                loss_source_domain_F = bce_loss(source_domain_gt, source_domain_feature_F)
                loss_target_domain_F = bce_loss(target_domain_gt, target_domain_feature_F)
                temporal_domain_loss = loss_source_domain_F + loss_target_domain_F

                loss_F_main = F.mse_loss(model_out_F, gt_frames_F)
                loss_F = loss_F_main + args.beta * temporal_domain_loss
                loss_F.backward()
                optimizer_F.step()              
            else:
                optimizer_F.zero_grad()
            ...
            losses_F_main.update(loss_F_main.item(),batch_size)
            losses_source_domain_F.update(loss_source_domain_F.item(),batch_size) 
            losses_target_domain_F.update(loss_target_domain_F.item(),batch_size) 
            # timing for fine net
            ...
            if batch_idx % args.log_interval == 0:
                logging.info(('Epoch: [{0}][{1}/{2}]\t'
                              'iters: {3}\t'
                              'Time {batch_time.val:.3f}\t[C:{batch_time_C.val:.3f}|F:{batch_time_F.val:.3f}] ({batch_time.sum:.3f}[{batch_time_C.sum:.3f}|{batch_time_F.sum:.3f}])\n'
                              'Loss_C {loss_C.val:.4f} ({loss_C.avg:.4f}) [SS:{loss_Ss.val:.4f}][ST:{loss_St.val:.4f}][MSE:{loss_Cm.val:.4f}({loss_Cm.avg:.4f})]\t\t'
                              'Loss_F {loss_F.val:.4f} ({loss_F.avg:.4f}) [TS:{loss_Ts.val:.4f}][TT:{loss_Tt.val:.4f}][MSE:{loss_Fm.val:.4f}({loss_Fm.avg:.4f})]\t'                              
                              .format(epoch, batch_idx, len(train_loader), global_step,
                                      ...
                                      loss_C=losses_C,
                                      loss_Ss=losses_source_domain,
                                      loss_St=losses_target_domain,
                                      loss_Cm=losses_C_main,
                                      loss_F=losses_F, 
                                      loss_Ts=losses_source_domain_F,
                                      loss_Tt=losses_target_domain_F,
                                      loss_Fm=losses_F_main)
                              ))

        if (epoch + 1) % 1 == 0:
            save_checkpoint(net_C.state_dict(),
                            path=args.checkpoint_dir_C,
                            filename="/C_checkpoints_small_{:03d}".format(epoch + 1) + ".pth.tar",
                            max_keep=20)
            # FREEZE F时不保存F模型
            save_checkpoint(net_F.state_dict(),
                            path=args.checkpoint_dir_F,filename="/F_checkpoints_small_{:03d}".format(epoch + 1) + ".pth.tar",
                            max_keep=20)
        ...
        if args.eval_on_the_fly and (epoch + 0) % args.eval_interval == 0:
            ...
            if avg_psnr_C > best_psnr_C:
                ...
                shutil.copyfile(src=args.checkpoint_dir_C + "/C_checkpoints_small_{:03d}".format(epoch + 1) + ".pth.tar",
                                dst=args.checkpoint_dir_best + "/C_checkpoints_best_" + str(epoch + 1) + ".pth.tar"
                                )
                logging.info('Best C PSNR {} found at {} epoch'.format(best_psnr_C, epoch))

            if avg_psnr_F > best_psnr_F:
                best_psnr_F = avg_psnr_F
                shutil.copyfile(src=args.checkpoint_dir_F + "/F_checkpoints_small_{:03d}".format(epoch + 1) + ".pth.tar",
                                dst=args.checkpoint_dir_best + "/F_checkpoints_best_" + str(epoch + 1) + ".pth.tar"
                                )
